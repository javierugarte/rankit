    ...
    >
      <div className="w-full max-w-lg bg-surface-2 rounded-t-3xl p-6 border-t border-border" style={{ paddingBottom: "max(1.5rem, env(safe-area-inset-bottom))" }}>
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-semibold text-text">{editList ? t("editTitle") : t("createTitle")}</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleCreate} className="space-y-5">
          ...
          <div>
            <label className="block text-xs text-muted mb-2 uppercase tracking-wider">
              {t("typeLabel")}{" "}
              <span className="normal-case text-muted/60">{t("typeOptional")}</span>
            </label>
            ...
            {listType && (
              <p className="text-xs text-muted mt-1.5">
                {t("autocompleteHint", { api: getService(listType)?.apiLabel ?? listType })}
              </p>
            )}
          </div>
